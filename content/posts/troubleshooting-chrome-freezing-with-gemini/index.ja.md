...
tags = ["AI", "Chrome", "Gemini", "トラブルシューティング"]
categories = ["AI 協作"]
...
私の職業病かもしれませんが、ソフトウェアが遅いと「待てばいい」とは思えない性格です。

ある日から、Windows で Chrome を起動すると必ず 1〜2 分間フリーズする、という不可解な現象に悩まされていました。待てば使えるものの、毎朝のこの「1 分間の沈黙」が地味にストレスで、根本原因の特定に乗り出すことにしました。
...
### 補足：ETW のサンプリング分析で分かること、分からないこと

UIforETW と WPA の組み合わせは、CPU のサンプリングに基づく分析手法です。一定間隔（通常 1ms）で「今 CPU がどの関数を実行しているか」を記録し、集計結果をフレームグラフやコールスタックとして表示します。

**この手法で分かること：**
- どのプロセス・スレッドが CPU 時間を占有しているか
- そのスレッド内で、どの関数が最も多くの CPU 時間を消費しているか

**この手法では分からないこと：**
- I/O 待ち（ディスクやネットワーク）が原因のフリーズ。CPU を使っていないため、サンプリングに現れない
- 関数の呼び出し回数そのもの（時間の占有率だけが分かる）

今回の現象は「フリーズ中に CPU 使用率が高い」タイプだったため、ETW のサンプリング分析が有効でした。もし CPU 使用率が低いままフリーズしていた場合は、Wait Analysis など別のアプローチが必要になります。
...
`CrBrowserMain`スレッドのコールスタックを深掘りしても、表示されるのは`chrome.dll!<SYMBOLS DISABLED>`ばかり。何度見ても答えは降ってきません。完全に手詰まりです。
