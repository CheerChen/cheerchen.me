...
tags = ["AWS", "Kubernetes", "cloud-native", "game-dev", "architecture", "AWS Summit Japan 2025"]
categories = ["tech"]
...
ソニー・インタラクティブエンタテインメント（SIE）による PSN 音声チャットサーバーの移行に関するセッションは、内容の薄い多くの講演の中で、その技術的な深さで強い印象に残りました。Kubernetes と格闘した日々を思い出しつつ、内容をまとめておきます。
...
- **1セッション = 1 Pod**：Agones は、個別のセッションごとに専用の Pod を割り当てる設計です。
...
- 1つの Pod 内部で、複数の独立したセッションが実行されます。要するに、Pod 内部で軽量なスケジューリング・管理レイヤーを実装していることになります。
