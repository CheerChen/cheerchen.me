...
tags = ["AWS", "ゲーム開発", "アーキテクチャ設計", "AWS Summit Japan 2025"]
categories = ["技術"]
...
- **クラスターの水平スケール:** 単一の EKS クラスターの Pod 数が数万に達し、コントロールプレーンが耐えきれなくなった際、彼らはクラスター自体の数を増やす（「_多い Region で 5 つ_」）という手法を採用し、単一リージョン内で複数の EKS クラスターを運用して負荷を分散させました。
...
- **核心的な動機:** 自前で Prometheus を構築するのは「_かなりのメモリ食い_」で管理が面倒であり、そのクエリ言語 PromQL を学ぶのも「_かなり大変_」。AWS のマネージドサービスを採用することで、チームは貴重なエネルギーをインフラ運用から解放し、ビジネスそのものに集中しました。
...
- **テスト方法:**
    ...
    - **改善サイクル:** 彼らの作業モデルは「負荷テストで問題を発見 → APM でボトルネックを特定 → 改善を行う」というクローズドループです。
- **最終成果:** グラフの緑色の線は着実に上昇し、最終的に「500 万同時接続相当まで達成」という数字に達しました。この数字は、これまで議論してきたすべてのアーキテクチャ設計（マイクロサービス、サービスメッシュ、リアルタイムサーバー、ハイブリッドデータベース）の最終的な価値を体現しています。
...
- **実例:** スライドでは具体的な例が挙げられています。「_監視のクォータが一部引っかかった_」。トラフィックのピーク時には、これは致命的になり得ます。
